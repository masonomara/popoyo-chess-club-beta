...
const TIME_CONTROLS: Record<
  Tables<"games">["time_control_category"],
  string[]
> = {
...
export default function AddGameSheet({
  ...
  async function uploadFile(file: File, label: string): Promise<string | null> {
    ...
    return supabase.storage.from("game-photos").getPublicUrl(path).data
      .publicUrl;
  }
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    formData.set(
      ...
      TC_CATEGORY_MAP.get(timeControl) ?? "rapid",
    );
